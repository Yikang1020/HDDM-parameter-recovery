...
def put_all_params_in_a_single_dict(joined_params, group_params, subj_noise, depends_on):
    ...
    if len(list(group_params.values())[0]) == 1:
        return p_dict

    #put subj params in p_dict
    for cond, cond_params in group_params.items():
        for idx, subj_dict in enumerate(cond_params):
            for (name, value) in subj_dict.items():
    ...
    for (name, value) in subj_noise.items():
...
def make_hash(o):
    ...
    try:
        ...
        return hashlib.md5(pickle.dumps(oo)).hexdigest()
